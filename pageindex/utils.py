...
class _TokenMeter:
    """Process-wide accumulator of LiteLLM token usage (thread-safe)."""
    ...
    def record(self, response):
        ...
        try:
            ...
            cost = 0.0  # provider pricing unknown (e.g. Copilot subscription)
        with self._lock:
...
token_meter = _TokenMeter()


@contextmanager
def track_usage():
    """Yield a dict that, on exit, holds the token usage accrued within the block.

    Computed as a delta over the process-wide meter, so it is accurate for
    sequential work (e.g. indexing one source). Concurrent LLM calls in other
    threads during the block would also be counted.
    """
